...
#' Here, we use a fine mesh of growth and beta rates to assess how much this 
#' affects results
setwd("../general_scripts/")
prior1 <- prior_obs_s_cont(n_ind,models=c(1,2),nsimul=c(nsim,nsim,0,0,0,0),
...
sims1 <- parApply(clu1,prior1,1,function(x){
  divfun_most(sim_seq(nsamp1 = x[1],theta1 = x[5],coal_param = x[3],model = x[2]),n_ind)})

stopCluster(clu1)

save(prior1,sims1,file=paste0("sims_rep",i,"/sim_m12_all_n",n_ind,
                              ".RData"))
}
