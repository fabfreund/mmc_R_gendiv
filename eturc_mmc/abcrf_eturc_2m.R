...
mods1 <- switch(col1,"red"=c(1,2),"green"=c(1,3),"lightblue"=c(1,3),"pink"=c(1,2),"kenya"=c(1,2)) 
nsim2 <- c(0,0,0,0,0,0)
nsim2[mods1][] <- n0
...
mods_mmc <- mods1[mods1 %in% 2:3] 

asmmc_error <- rf_ms$model.rf$confusion.matrix["1",as.character(mods_mmc)]/sum(rf_ms$model.rf$confusion.matrix["1",as.character(mods1)])

askm_error <- 
  sum(rf_ms$model.rf$confusion.matrix[as.character(mods_mmc),"1"])/sum(rf_ms$model.rf$confusion.matrix[as.character(mods_mmc),as.character(mods1)])
...
if (log_arg){
write.table(out1,file=paste0("../eturc_mmc/res2m/res2m_",names(data_scaffs)[i],
                             ".txt"),quote = FALSE,row.names = FALSE)} else {
write.table(out1,file=paste0("../eturc_mmc/res2/res2m_",names(data_scaffs)[i],
