...
#' 3 Args: Which data set from mtb_data_call90.RData, 
...
source("serial_sampling_priorsim.R")
...
growthrange0 <- c(0,5000)

betarange0 <- c(1,2)
diracrange0 <- c(0,1)
n0 <- 125000#80000 for the reduced set
best_fit <- c(rep(2,8),3,2,1,2,2,rep(3,3),rep(2,4),1,2,1)
mod1 <- best_fit[i]
...
prior2 <- prior_obs_s_mmc_tc(n_ind = data_main[[i]]$n_ind,
                             nsimul = n0,coal_p = c(1,1.9),coal_m = "beta",
                             expg_p = beta_gran1,s_obs = data_main[[i]]$n_mut,
...
prior3 <- prior_obs_s_mmc_tc(n_ind = data_main[[i]]$n_ind,
                             nsimul = n0,coal_p = c(0.05,0.95),coal_m = "dirac",
                             expg_p = dirac_gran1,s_obs = data_main[[i]]$n_mut,
                             mc1 = mc1,discr = TRUE,log_g = TRUE)      
...
divfun_ohaf(Lambda_seq_sim_difft(
...
save(prior1,prior2,prior3,sims1,file=paste0("sims/sim_mmcg_",names(data_main)[i],".RData"))
...
save(data2model,conf.matrix,file=paste0("resdata/abcout",rep1,"_",names(data_main)[i],"_mmcg.RData"))
