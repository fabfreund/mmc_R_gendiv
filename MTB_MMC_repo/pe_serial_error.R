...
#' Run via Rscript, additional argument integer 1-3
#' (1=Eldholm 2015, 2=Lee 2015, 3=Roetzer 2013) 
#' Error of parameter estimation (average absolute deviation 
#' true vs. posterior median).
...
prior_rf <- prior_obs_s(params_serial[[i]]$n_ind,models=c(1,2,3,6),
...
pe_rf_serial <- vector("list",length(sims1))
names(pe_rf_serial) <- names(sims1)

err_rf_serial <- pe_rf_serial


#' Sims to build the regression forests
model1 <- prior_rf[,2]
coal_p <- prior_rf[,3]

sumstat_rf <- t(sims_rf)
rownames(sumstat_rf) <- NULL



for (k in 1:length(sims1)){
# For each parameter combination of serial coalescent simulation,
# built random forest for ABC parameter estimation 
  param_rf <- coal_p[model1==k]
  sumstat2 <- data.frame(param_rf,sumstat_rf[model1==k,])
  paramest_rf <- regAbcrf(param_rf~.,
                        data=sumstat2,
                        paral = TRUE,ncores = mc1)
  
  
pe_rf_serial[[k]] <- vector("list",nrow(params_order[[k]]))  
...
obs_data <- as.data.frame(t(sims1[[k]][[j]])) 
obs_data <- obs_data[apply(obs_data,1,function(v){!(any(is.na(v)))}),]

#' Estimate parameters via random forest
pe_result <- predict(paramest_rf,obs_data,
                     sumstat2,ncores=mc1,
                     quantiles = c(0,0.025,0.975,1))

pe_rf_serial[[k]][[j]] <- pe_result

if (k==1){res1 <- mean(abs(pe_result$med - params_order[[k]][j,1]))/params_order[[k]][j,1]} else {
  res1 <- mean(abs(pe_result$med - params_order[[k]][j,1]))}

err_rf_serial[[k]][as.character(params_order[[k]][j,2]),
                   as.character(params_order[[k]][j,1])] <- res1
                            


}  
}
save(err_rf_serial,pe_rf_serial,
     file=paste0("resdata/serial_pe_",names(params_serial)[i],".RData"))
